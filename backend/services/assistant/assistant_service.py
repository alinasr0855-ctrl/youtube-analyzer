...
SOLID principles applied:
  S — Each collaborator (Classifier, Router, RAG, Gemini) has one job.
  O — New modes added via ExecutionMode enum without touching this class.
  L — Gemini and RAG services are interchangeable via the same _generate() call.
  I — AssistantService only depends on the narrow interfaces it needs.
  D — Depends on abstract service functions, not concrete implementations.
...
Flow:
  ...
  3. Execute path:
       ...
       HYBRID      → rag_service.retrieve_context()
                     ...
                     + gemini_service._call_model()  [single LLM call]
...
class AssistantService:
    ...
    # ── Path 1: Gemini (analysis-based, no retrieval) ─────────────────────────
    ...
    # ── Path 2: RAG-only (transcript retrieval + strict prompt) ───────────────
    def _rag_only(self, question, session, history, video_ids) -> Dict:
        try:
            ...
            # Auto-fallback: if retrieval returned nothing, use Gemini
            if r.get("retrieval_count", 0) == 0:
                from backend.services import cache_service
                analyzed = [v for v in cache_service.load_results(session["session_id"])
                            if v.get("analyzed")]
            ...
            # RAG completely failed → Gemini fallback
            from backend.services import cache_service
            analyzed = [v for v in cache_service.load_results(session["session_id"])
                        if v.get("analyzed")]
            fallback = self._gemini(question, session, analyzed, history)
    ...
    # ── Path 3: Hybrid (retrieve context → single Gemini call with both sources)
    def _hybrid(self, question, session, history, analyzed_videos, video_ids) -> Dict:
        try:
            # Retrieve transcript chunks (no generation yet)
            ctx, results = self.rag.retrieve_context(question, session, video_ids)

            # If retrieval empty → fall back to Gemini
            if not results:
            ...
            # Build hybrid prompt (transcript + general knowledge allowed)
            ...
            return {
                ...
                "mode":              "hybrid",
                "sources":           ctx.sources,
                "referenced_videos": self.rag._video_refs(ctx.sources),
                "confidence":        conf,
