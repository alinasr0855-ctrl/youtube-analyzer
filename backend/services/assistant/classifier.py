...
class QuestionClassifier:
    ...
    def classify(self, question: str) -> Classification:
        ...
        total = vs + gs + hs + 1e-9
        vs_n = vs/total; gs_n = gs/total; hs_n = hs/total
        ...
        if hs > 0:
            ...
            if vs > gs * self.DOMINANCE_RATIO:
                qtype = QuestionType.VIDEO_SPECIFIC; confidence = min(0.95, vs_n+0.2)
                reason = f"Video signals dominate (vs={vs:.2f} > gs={gs:.2f})"
            elif gs > vs * self.DOMINANCE_RATIO:
                qtype = QuestionType.GENERAL; confidence = min(0.95, gs_n+0.2)
                reason = f"General signals dominate (gs={gs:.2f} > vs={vs:.2f})"
            else:
                qtype = QuestionType.HYBRID; confidence = 0.60
                reason = f"Mixed signals (vs={vs:.2f}, gs={gs:.2f})"
        elif vs > self.MIN_SIGNAL_SCORE:
            qtype = QuestionType.VIDEO_SPECIFIC; confidence = min(0.90, vs_n+0.25)
            reason = f"Video-specific signals (score={vs:.2f})"
        elif gs > self.MIN_SIGNAL_SCORE:
            qtype = QuestionType.GENERAL; confidence = min(0.90, gs_n+0.25)
            reason = f"General knowledge signals (score={gs:.2f})"
        else:
            qtype = QuestionType.HYBRID; confidence = 0.50
            reason = "No strong signals — defaulting to Hybrid for best coverage"

        return Classification(question_type=qtype, confidence=round(confidence,3),
                              video_score=round(vs,4), general_score=round(gs,4), reason=reason)
