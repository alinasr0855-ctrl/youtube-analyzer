...
Open/Closed: add new prompt types without modifying existing methods.
"""
...
class PromptEngineer:
    ...
    # ── RAG-only prompt (strict: transcript only) ──────────────────────────────
    ...
    # ── Hybrid prompt (transcript + general knowledge) ─────────────────────────
    ...
    # ── Internal builder (shared, no duplication) ──────────────────────────────
    def _assemble(
        self,
        system: str,
        question: str,
        ...
        if session_metadata:
            pl = session_metadata.get("playlist_name", "")
            ch = session_metadata.get("channel_name", "")
            if pl:
